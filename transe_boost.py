...
class TransEBoost2():
    def __init__(self, model, train_data, start_epoch, end_epoch, seed, device, batch_size, start_model, end_model,
                 optimizer):
        self.model = model
        self.train_data = train_data
        self.start_epoch = start_epoch
    ...
    def evaluate(self, data, print_cond=False, get_index=False):
        #print('Starting evaluation for TransE boosting: ')
        rank_count=0
        self.tail_dict = {}
        self.head_dict = {}
    ...
    def train(self):
        ...
        err_index = torch.arange(0, self.train_data.shape[0], dtype=torch.int64)
        for model in range(self.start_model, self.end_model):
            ...
            err_index = self.evaluate(self.train_data[err_index,:], get_index=True)
